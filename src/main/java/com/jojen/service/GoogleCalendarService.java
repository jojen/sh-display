...
public class GoogleCalendarService {
    ...
    String calendarId;
    @Value("${google.taskcalendar.id}")
    String taskCalenderId;
    ...
    public List<String> getUpcomingTasks() throws IOException {
        List<String> ret = new ArrayList<>();
        ZoneId defaultZoneId = ZoneId.systemDefault();
        Date maxTime = Date.from(LocalDateTime.now().plusDays(2).toLocalDate().atStartOfDay(defaultZoneId).toInstant());

        Events events = googleCalendar.events().list(taskCalenderId)
                .setTimeMin(new DateTime(new Date()))
                .setTimeMax(new DateTime(maxTime))
                .setMaxResults(6)
                .setOrderBy("startTime")
